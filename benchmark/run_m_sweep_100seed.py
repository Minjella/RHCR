#!/usr/bin/env python3
"""
m-sweep (100 seeds) — ECBS vs PBS(+opts) vs PBSSection scaling across agent count.

Sweep:
  m in {300, 400, 500, 600, 700, 800, 900}  (900 conditional: see ENABLE_M900)
  w in {10, 100}   # 10 = best PBS-Section operating point, 100 = asymptotic
  solvers: ECBS(1.1), PBS+opts (baseline mode), PBSSection (BEST_BUDGET=10000)
  seeds: 0..99

Total (w/o m=900): 3 x 2 x 6 x 100 = 3600 jobs
Total (w/ m=900): 3 x 2 x 7 x 100 = 4200 jobs

Output:
  benchmark_results_msweep100/logs/{label}_w{w}_m{m}_s{seed}.log
  benchmark_results_msweep100/per_run.csv, summary.txt
"""
import csv
import os
import re
...
OUT_DIR = REPO / "benchmark_results_msweep100"
LOG_DIR = OUT_DIR / "logs"

BEST_BUDGET = 10000  # from benchmark_results_budget100

WINDOWS = [5, 10, 100]
AGENTS = [300, 400, 500, 600, 700, 800]
ENABLE_M900 = False
if ENABLE_M900:
    AGENTS.append(900)
...
SUBOPT = 1.1
PARALLEL = 10
TIMEOUT_SEC = 90 * 60  # 90 min per run
...
def run_single(args):
    label, solver, mode, _prefix, w, m, seed = args
    tag = f"{label}_w{w}_m{m}_s{seed}"
    ...
    try:
        ...
        return label, w, m, seed, elapsed, result.returncode
    except subprocess.TimeoutExpired:
        elapsed = time.time() - t0
        log_file.write_text(f"TIMEOUT after {elapsed:.1f}s")
        return label, w, m, seed, elapsed, -999


def parse_log(label, prefix, w, m, seed):
    log_file = LOG_DIR / f"{label}_w{w}_m{m}_s{seed}.log"
...
def summarize(rows, label, w, m):
    sel = [r for r in rows if r["solver_label"] == label and r["window"] == w and r["m"] == m]
    ok = [r for r in sel if r["sim_complete"]]
    s = {"solver": label, "window": w, "m": m,
...
def main():
    ...
    # Easiest first: small m, small w, PBSSection first (cheap).
    order = {"PBS_section": 0, "ECBS_baseline": 1, "PBS_opts": 2}
    jobs.sort(key=lambda j: (j[5], j[4], order[j[0]], j[6]))

    print(f"Running {len(jobs)} jobs, parallel={PARALLEL}")
    print(f"m={AGENTS}  w={WINDOWS}  seeds={len(SEEDS)}  sim_time={SIM_TIME}  BEST_BUDGET={BEST_BUDGET}")
    sys.stdout.flush()
    ...
    with ProcessPoolExecutor(max_workers=PARALLEL) as ex:
        ...
        for f in as_completed(futures):
            label, w, m, seed, elapsed, rc = f.result()
            wall[(label, w, m, seed)] = elapsed
            done += 1
            line = (f"[{done}/{len(jobs)}] {label:14s} w={w:3d} m={m} s={seed:3d} "
    ...
    rows = []
    for (label, _s, _m, prefix, w, m, seed) in jobs:
        r = parse_log(label, prefix, w, m, seed)
        if r:
    ...
    lines = []
    lines.append(f"m-sweep (100 seeds): 3 solvers x {len(WINDOWS)} windows x {len(AGENTS)} m values")
    lines.append(f"w={WINDOWS}  m={AGENTS}  sim_time={SIM_TIME}  subopt(ECBS)={SUBOPT}  BEST_BUDGET={BEST_BUDGET}")
    lines.append(f"Total wall: {total:.1f}s ({total/3600:.2f} h)")
    lines.append("")
    for s in summaries:
        lines.append(f"=== {s['solver']} w={s['window']} m={s['m']} ===")
