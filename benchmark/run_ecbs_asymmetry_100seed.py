#!/usr/bin/env python3
"""
ECBSSection asymmetry experiment (100 seeds).

Demonstrates that the Section abstraction interacts asymmetrically with HL strategy:
it accelerates priority-based search (PBSSection) but degrades constraint-based
search (ECBSSection).

Sweep:
  ECBS_baseline (1.1)  vs  ECBSSection
  m = 500, w ∈ {5, 10}, 100 seeds = 400 runs
"""
import csv
import os
import re
...
OUT_DIR = REPO / "benchmark_results_ecbs_asymmetry"
...
SOLVERS = [
    ...
    ("ECBS_section",  "ECBS", "section",  "ECBSSection:"),
]
...
def main():
    ...
    # ECBS baseline first (faster), section later
    order = {"ECBS_baseline": 0, "ECBS_section": 1}
    jobs.sort(key=lambda j: (order[j[0]], j[4], j[5]))

    print(f"Running {len(jobs)} jobs, parallel={PARALLEL}")
    print(f"w={WINDOWS}  m={AGENTS}  seeds={len(SEEDS)}  sim_time={SIM_TIME}  subopt={SUBOPT}")
    sys.stdout.flush()
    ...
    lines = []
    lines.append(f"ECBSSection asymmetry (100 seeds): ECBS_baseline vs ECBSSection")
    lines.append(f"w={WINDOWS}  m={AGENTS}  sim_time={SIM_TIME}  subopt={SUBOPT}")
